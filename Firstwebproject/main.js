...
const NotificationManager = {
  ...
  async sendEmailNotification(task) {
    ...
    // Only send if exactly 2 hours before deadline
    if (timeUntilDeadline <= 7200000 && timeUntilDeadline > 7140000) { // Between 2 hours and 1 hour 59 minutes
      try {
        ...
        console.log('2-hour reminder email sent for:', task.title);
        return true;
      } catch (error) {
  ...
  scheduleNotification(task) {
    ...
    // Schedule email notification exactly 2 hours before
    const emailDelay = notifyTime - now;
    if (emailDelay > 0) {
      setTimeout(() => {
