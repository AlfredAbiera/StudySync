...
async function checkNotifications() {
  try {
    ...
    console.log(`Checking ${pendingNotifications.length} pending notifications at ${new Date(now).toLocaleTimeString()}`);
    
    // For each pending notification, check if it should be shown
    for (const notification of pendingNotifications) {
      const timeUntilDeadline = notification.deadline - now;
      
      // Show notification if it's within 2 hours and hasn't been shown yet
      if (!notification.shown && timeUntilDeadline <= 7200000 && timeUntilDeadline > 0) {
